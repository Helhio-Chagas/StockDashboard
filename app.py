...
def build_sidebar():
    ...
    ticker_list = pd.read_csv("ticket_ibra.csv", index_col=1, sep=";")
    tickers = st.multiselect(label="Selecione os Ativos", options=ticker_list, placeholder="Ativos")
...
def build_main(tickers, prices):
    ...
    mygrid = grid(5, 5, 5, 5, 5, 5, 5, vertical_align="top")
    for t in prices.columns:
        c = mygrid.container(border=True)
        c.subheader(t, divider="red")
        colA, colB, colC = c.columns(3)
        if t=="Portfolio":
            colA.image("image/market.png")
        elif t=="Ibov":
            colA.image("image/grafico.png")
        else:
            colA.image(f"https://raw.githubusercontent.com/thefintz/icones-b3/main/icones/{t}.png",width=75)
